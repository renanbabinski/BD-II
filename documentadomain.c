...
/*
A função abaixo quebra a string sql em substrings para identificar qual operação o usuário deseja executar
*/
...
/*
A função abaixo cria uma nova tabela se for solicitada pela função insertInto
no caso de não haver espaço suficiente na pagina anterior
Define-se nome de pagina, tamanho, memória livre e quantos itens existem nela
*/
...
Essas informações são gravadas na pagina header(cabeçalho)
*/
...
void buildHeader(char *sql, char *tableName, int qtdPages){
  ...
  fwrite(&i, sizeof(int), 1, headerPage); //escreve endereço de i na pagina
  ...
  fwrite(&i, sizeof(int), 1, headerPage);  //grava endereço de i no arquivo
...
/*
A função abaixo insere novas variveis em atributos de uma tabela ja criada
Calcula tamanho necessário conforme os dados passados pelo usuário, se é possivel inserir na pagina atual ou se
é necessário criar uma nova
Após inserir atualiza os dados da pagina na estrutura header
*/
void insertInto(char *sql, int numPage){
  ...
  for(int i = 0; i < qtdFields; i++){  //Aqui é calculado o tamanho total necessário para a inserção
  ...
  if(head.memFree > insertSize){    //se o tamanho total da inserção for menor que o espqço livre, insira aqui mesmo
    token = strtok(attrSqlCopy, ",");
    item newItem;
    ...
    fseek(page, nextItem, SEEK_SET);   //seta ponteiro no proximo espaço livre
    ...
    fseek(page, 0, SEEK_SET);  //seta ponteiro no inicio da pagina
 
    fwrite(&head.memFree, sizeof(int), 1, page); //novos dados da pagina são gravados
    fwrite(&head.next, sizeof(int), 1, page);
    fwrite(&head.qtdItems, sizeof(int), 1, page);

    fclose(page); //fecha pagina

    printf("New item inserted\n");
  }else {                                //Se não houver espaço cria nova pagina
    ...
    if(special == '0'){
      ...
      //printf("SQL - %s\n", sql);
      createPage(tableName, nextPage);  //criar nova pagina
      special = '1';
      fseek(page, 8191, SEEK_SET);
      ...
      fclose(page);
      insertInto(sql, nextPage);  //recursão para inserção do item na nova pagina
    }
  }
}
...
/*
A função abaixo seleciona atributos de uma tabela predefinida ja criada,
le informações sobre os atrubutos como nomes, tamanhos e tipos de variaveis
para que a função saiba quantos caracteres deve ler até a parada no arquivo
*/ 
...
void selectFrom(char *sql, int numPage){
  ...
  for(int i = 0; i < qtdFields; i++){   //nomes, tamanhos, e tipos de atributos a  serem lidos
    fread(attributes[i].name, 15, 1, headerPage);
  ...
  for(int i = 0; i < head.qtdItems; i++){
    ...
    for(int j = 0; j < qtdFields; j++){   //escreve variaveis char
      if(attributes[j].type == 'C'){
        ...
        printf("\t");
      } else if(attributes[j].type == 'I'){   //escreve variaveis int
        fread(&intInFile, sizeof(int), 1, page);
        printf("%d\t", intInFile);
      } else if(attributes[j].type == 'V'){   //escreve variveis varchar e para quando encontrar "$"
        charInFile = ' ';
        do{
            fread(&charInFile, 1, 1, page);
            if(charInFile != '$')
...
/*
A função deleteFrom abaixo recebe a string de comando SQL e o numero da pagina a qual começar a buscar
Através de condições e loops determina-se a origem e o final dos dados escritos na pagina
para que seja possivel apaga-los de forma correta sem comprometer outros dados
Se a pagina ficar vazia após uma deleção ela é apagada
*/
